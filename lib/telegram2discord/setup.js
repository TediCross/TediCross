 ...
 * @private
 */
const createMessageHandler = R.curry((logger, tgBot, bridgeMap, func, ctx) => {
	if (ctx.message.text === "/chatinfo") {
		...
		// Get the bridge
		const bridges = bridgeMap.fromTelegramChatId(ctx.message.chat.id);
		...
		if (bridges === undefined) {
			...
			tgBot.telegram.sendMessage(
				...
				.catch((err) => {
					...
					logger.error("Could not tell user to get their own TediCross instance:", err, ctx.message);
				});
		} else {
...
function setup(logger, tgBot, dcBot, messageMap, bridgeMap, settings) {
	...
	tgBot.on("text", wrapFunction(async ({ message }, bridge) => {
		...
		try {
			...
			if (messageObj.reply !== null) {
				...
				await channel.send(textToSend, {embed});
			}

			// Discord doesn't handle messages longer than 2000 characters. Split it up into chunks that big
			const messageText = (!bridge.telegram.sendUsernames || messageObj.reply !== null)
				? messageObj.text
				: `**${messageObj.from}**\n${messageObj.text}`
			;
	...
	tgBot.on("newParticipants", wrapFunction(({ message: { new_chat_members } }, bridge) => {
	...
	tgBot.on("participantLeft", wrapFunction(async ({ message: { left_chat_member } }, bridge) => {
		// Ignore it if the settings say no
	...
	tgBot.on("messageEdit", wrapFunction(async (ctx, bridge) => {
