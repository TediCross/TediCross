...
const createMessageHandler = R.curry((logger, tgBot, bridgeMap, func, message) => {
	if (message.text === "/chatinfo") {
		...
		if (bridges === undefined) {
			...
			tgBot.sendMessage({
				chat_id: message.chat.id,
				text: "This is an instance of a [GoatBot](https://github.com/Myke500/GoatBot) bot, "
					+ "bridging a chat in Telegram with one in Discord. "
					+ "If you wish to use GoatBot on your own server, please download and create an instance. "
				,
				parse_mode: "markdown"
			})
				.catch((err) => {
					// Hmm... Could not send the message for some reason
					logger.error("Could not tell user to get their own GoatBot instance:", err, message);
				});
		} else {
