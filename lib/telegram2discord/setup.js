 ...
 * @private
 */
const createMessageHandler = R.curry((tgBot, func, ctx) => {
...
function setup(logger, tgBot, dcBot, messageMap, bridgeMap, settings) {
	...
	tgBot.on("text", wrapFunction(async (ctx, bridge) => {
	...
	tgBot.on("photo", wrapFunction(async (ctx, bridge) => {
	...
	tgBot.on("sticker", wrapFunction(async (ctx, bridge) => {
	...
	tgBot.on("document", wrapFunction(async (ctx, bridge) => {
	...
	tgBot.on("voice", wrapFunction(async (ctx, bridge) => {
	...
	tgBot.on("audio", wrapFunction(async (ctx, bridge) => {
	...
	tgBot.on("video", wrapFunction(async (ctx, bridge) => {
	...
	tgBot.on("new_chat_members", wrapFunction((ctx, bridge) => {
	...
	tgBot.on("left_chat_member", wrapFunction(async (ctx, bridge) => {
	...
	const handleEdits = wrapFunction(async (ctx, bridge) => {
		try {
