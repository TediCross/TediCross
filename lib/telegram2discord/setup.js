 ...
 */
function setup(tgBot, dcBot, logger, settings, dcUsers, messageMap) {
	// Start longpolling
	...
	tgBot.on("text", wrapFunction(message => {
		...
		let text = handleEntities(dcUsers, settings, message.text, message.entities);
		...
		dcBot.channels.get(settings.discord.channelID).send(`**${fromName}**: ${text}`)
		.then(dcMessage => {
	...
	tgBot.on("photo", wrapFunction(message => {
		...
		let caption = handleEntities(message.caption, message.entities);
