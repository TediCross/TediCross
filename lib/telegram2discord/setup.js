...
function setup(tgBot, dcBot, dcUsers, messageMap) {
	...
	tgBot.on("photo", wrapFunction((message, bridge) => {
		sendFile({
		  ...
		  .catch((err) => console.error("Could not send photo", err));
	}));
	...
	tgBot.on("sticker", wrapFunction((message, bridge) => {
		sendFile({
		  ...
		  .catch((err) => console.error("Could not send sticker", err));
	}));
	...
	tgBot.on("document", wrapFunction((message, bridge) => {
		sendFile({
		  ...
		  .catch((err) => console.error("Could not send document", err));
	}));
	...
	tgBot.on("audio", wrapFunction((message, bridge) => {
		sendFile({
		  ...
		  .catch((err) => console.error("Could not send audio", err));
	}));
	...
	tgBot.on("video", wrapFunction((message, bridge) => {
		sendFile({
		  ...
		  .catch((err) => console.error("Could not send video", err));
	}));
