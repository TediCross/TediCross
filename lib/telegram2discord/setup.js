...
function setup(tgBot, dcBot, dcUsers, messageMap) {
	...
	tgBot.on("photo", wrapFunction(message => {
		...
		let caption = handleEntities(dcUsers, message.caption, message.entities);
		...
		tgBot.getFile({file_id: message.photo[message.photo.length-1].file_id})
		  ...
		  .catch(err => {
			Application.logger.log("Something went wrong when relaying a photo from Telegram to Discord:", err);
		  });
	}, tgBot));
		...
		tgBot.getFile({file_id: message.document.file_id})
		  ...
		  .then(fileStream => {
			...
			fileStream.on("end", () => {
				...
				.catch(err => Application.logger.error("Discord did not accept a document:", err));
			});
		  })
		  .catch(err => {
			Application.logger.log("Something went wrong when relaying a document from Telegram to Discord:", err);
		  });
	});
		...
		tgBot.getFile({file_id: message.audio.file_id})
		  ...
		  .then(fileStream => {
			...
			fileStream.on("end", () => {
				...
				.catch(err => Application.logger.error("Discord did not accept an audio file:", err));
			});
		  })
		  .catch(err => {
			Application.logger.log("Something went wrong when relaying an audio file from Telegram to Discord:", err);
		  });
	});
		...
		tgBot.getFile({file_id: message.video.file_id})
		  ...
		  .then(fileStream => {
			...
			fileStream.on("end", () => {
				...
				.catch(err => Application.logger.error("Discord did not accept a video:", err))
			});
		  })
		  .catch(err => {
			Application.logger.log("Something went wrong when relaying a video from Telegram to Discord:", err);
		  });
	});
		...
		tgBot.getFile({file_id: message.sticker.thumb.file_id})
		  ...
		  .then(fileStream => {
			...
			fileStream.on("end", () => {
				...
				.catch(err => Application.logger.error("Discord did not accept a sticker:", err))
			});
		  })
		  .catch(err => {
			Application.logger.log("Something went wrong when relaying a sticker from Telegram to Discord:", err);
		  });
	}, tgBot));
