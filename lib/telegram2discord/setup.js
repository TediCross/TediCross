...
function setup(tgBot, dcBot, dcUsers, messageMap, bridgeMap, settings) {
	...
	updateEmitter.on("text", wrapFunction((message, bridge) => {
		...
		dcBot.ready.then(() => {
			return dcBot.channels.get(bridge.discordChannelId).send(messageObj.composed);
		  })
		  .then((dcMessage) => {
	...
	updateEmitter.on("photo", wrapFunction((message, bridge) => {
		sendFile({
			discordChannel: bridge.discordChannelId,
			message,
	...
	updateEmitter.on("newParticipants", wrapFunction(({new_chat_members}, bridge) => {
		...
		if (!bridge.telegram.relayJoinLeaveMessages) {
			return;
		}
		...
		new_chat_members.forEach((user) => {
			...
			// Pass it on
			dcBot.ready.then(() => {
				return dcBot.channels.get(bridge.discordChannelId).send(text);
			  })
	...
	updateEmitter.on("messageEdit", wrapFunction((tgMessage, bridge) => {
		...
		dcBot.ready
		  ...
		  .then((dcMessageId) => dcBot.channels.get(bridge.discordChannelId).fetchMessage(dcMessageId))
		  .then((dcMessage) => {
