...
function makeFileSender(tgBot, dcBot, dcUsers, settings) {
	 ...
	 */
	return function(bridge, {message, fileId, fileName, caption = "", resolveExtension = false}) {
		...
		return dcBot.ready
			...
			.then((fileStream) => new Promise((resolve) => {
				...
				fileStream.on("end", () => {
					const p = dcBot.channels.get(bridge.discord.channelId).send(
						textToSend,
						{
							file: {
	...
	updateEmitter.on("text", wrapFunction((message, bridge) => {
		...
		dcBot.ready.then(() => {
			...
			return chunks.reduce((p, chunk) => {
				return p.then(() => channel.send(chunk));
			}, Promise.resolve());
		})
			.then((dcMessage) => {
