...
// TODO Temporary logging to see what's going on. Remove before release
tgBot.use((ctx, next) => {console.log(ctx.tediCross); next();});
			...
			tgBot.on("edited_message", endwares.handleEdits);
			tgBot.on("edited_channel_post", endwares.handleEdits);
			R.forEach(updateType => tgBot.on(updateType, endwares.relayMessage), [
				"text",
				"audio",
				"document",
				"photo",
