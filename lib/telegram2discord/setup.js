...
function setup(logger, tgBot, dcBot, messageMap, bridgeMap, settings) {
	tgBot.ready = tgBot.telegram.getMe()
		.then(me => {
			...
			tgBot.context.self = me; // XXX For some strange reason Telegraf crashes if I try to call the prop `me`
			...
			tgBot.use(middlewares.addFromObj); // Not used at the moment
			tgBot.use(middlewares.addReplyObj); // Not used at the moment
			tgBot.use(middlewares.addForwardFrom); // Not used at the moment
			tgBot.use(middlewares.addTextObj); // Not used at the moment
			...
			tgBot.on("text", createMessageHandler(async (ctx, bridge) => {
			...
			tgBot.on("photo", createMessageHandler(async (ctx, bridge) => {
			...
			tgBot.on("sticker", createMessageHandler(async (ctx, bridge) => {
			...
			tgBot.on("document", createMessageHandler(async (ctx, bridge) => {
			...
			tgBot.on("voice", createMessageHandler(async (ctx, bridge) => {
			...
			tgBot.on("audio", createMessageHandler(async (ctx, bridge) => {
			...
			tgBot.on("video", createMessageHandler(async (ctx, bridge) => {
