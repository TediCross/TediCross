	...
	for (let i = entities.length-1; i >= 0; i--) {
		...
		switch (e.type) {
			...
			case "text_mention": {
				...
				const dcUser = dcBot.channels.get(bridge.discord.channelId).members.find(findFn("displayName"));
				const dcRole = dcBot.guilds.get(bridge.discord.serverId).roles.find(findFn("name"));
				if (!R.isNil(dcUser)) {
