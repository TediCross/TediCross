 ...
 */
function handleEntities(dcUsers, settings, text, entities = []) {
	...
	for (let i = entities.length-1; i >= 0; i--) {
		...
		switch(e.type) {
			...
			case "pre":
				...
				break;
			case "text_link":
				// Link with text. 'part' is the text, 'e.url' is the URL
//				substitute = "[" + part + "](" + e.url + ")";
				// Discord appears to not be able to handle this type of links. Just substitute the URL
				substitute = e.url;
				break;
			case "bold":
				// Bold text
