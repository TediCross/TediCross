 ...
 */
function messageConverter(ctx, tgBot, settings, dcBot, bridge) {
	...
	if (!R.isNil(ctx.tediCross.replyTo)) {
		...
		if (message.reply_to_message.from !== undefined && message.reply_to_message.from.id === tgBot.context.TediCross.me.id) {
			// Get the name of the Discord user this is a reply to
			const dcUsername = message.reply_to_message.text.split("\n")[0];
			const dcUser = dcBot.channels.get(bridge.discord.channelId).members.find(findFn("displayName", new RegExp(dcUsername)));
		...
		if (message.reply_to_message.text !== undefined) {
			...
			originalText = originalText.length > settings.discord.replyLength
				? originalText.slice(0, settings.discord.replyLength) + "…"
				: originalText
			;
