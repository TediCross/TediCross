 ...
 * @private
 */
function getDisplayName(user, chat, useFirstNameInsteadOfUsername, addLastName) {
	return chat.type === "channel"
		? getDisplayNameFromChannel(chat)
		: getDisplayNameFromUser(user, useFirstNameInsteadOfUsername, addLastName)
	;
}
 ...
 */
function getDisplayNameFromUser(user, useFirstNameInsteadOfUsername, addLastName) {
	...
	if (!displayName || useFirstNameInsteadOfUsername) {
		...
		// If wished we also add the last name of the telegram user
		if (addLastName && user.last_name) {
...
function messageConverter(message, tgBot, settings, dcBot, bridge) {
	...
	let fromName = getDisplayName(message.from, message.chat, settings.telegram.useFirstNameInsteadOfUsername, settings.telegram.addLastName);
	...
	if (message.reply_to_message !== undefined) {
		...
		if (message.reply_to_message.from !== undefined && message.reply_to_message.from.id === tgBot.me.id) {
			...
			inReplyTo = dcUser !== undefined ? `<@${dcUser.id}>` : dcUsername;
		}
