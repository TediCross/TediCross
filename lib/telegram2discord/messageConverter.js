 ...
 */
function messageConverter(message, dcUsers, tgBot, settings, dcBot, bridge) {
	// Convert the text to Discord format
	let text = handleEntities(dcUsers, message.text, message.entities, dcBot, bridge);
	...
	if (message.reply_to_message !== undefined) {
		...
		if (message.reply_to_message.text !== undefined) {
			...
			repliedToText = repliedToText.length > 100
				...
				: repliedToText
			;
			let newlineIdices = [...repliedToText].reduce((indices, c, i) => {
				if (c === "\n") indices.push(i);
				return indices;
			}, []);
			if (newlineIdices.length > 1) {
				repliedToText = repliedToText.slice(0, newlineIdices[1]) + "…";
			}
