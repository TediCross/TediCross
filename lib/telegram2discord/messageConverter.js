...
function messageConverter(message, tgBot, settings, dcBot, bridge) {
	...
	if (message.reply_to_message !== undefined) {
		...
		if (message.reply_to_message.from !== undefined && message.reply_to_message.from.id === tgBot.me.id) {
			...
			const dcUsername = new RegExp(message.reply_to_message.text.split("\n")[0]);
			const dcUser = dcBot.channels.get(bridge.discord.channelId).members.find(findFn("displayName", dcUsername));
