...
function addReplyObj(ctx, next) {
	...
	if (!R.isNil(repliedToMessage)) {
		...
		ctx.tediCross.replyTo = {
			...
			text: R.defaultTo(undefined, createTextObjFromMessage(repliedToMessage)),
			isReplyToTediCross: !R.isNil(repliedToMessage.from) && R.equals(repliedToMessage.from.id, ctx.TediCross.me.id)
		};
	}
