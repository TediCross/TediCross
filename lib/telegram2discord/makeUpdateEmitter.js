 ...
 * @private
 */
async function fetchUpdates(bot, offset, emitter, debug) {
	...
	try {
		...
		Application.logger.error("Couldn't fetch Telegram messages. Reason:", `${err.name}: ${err.message}` + debug ? err.stack : "");
	} finally {
		// Do it again, no matter what happened. XXX There is currently no way to stop it
		fetchUpdates(bot, offset, emitter, debug);
	}
}
...
function makeUpdateEmitter(bot, settings) {
	...
	// Start the fetching
	p.then(() => fetchUpdates(bot, 0, emitter, settings.debug));
