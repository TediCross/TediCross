 ...
 * @private
 */
async function fetchUpdates(logger, bot, offset, emitter, debug) {
	// Log the event if debugging is on
	if (debug) {
		logger.log("Fetching Telegram updates");
	}
	...
	try {
		...
		logger.error("Couldn't fetch Telegram messages. Reason:", `${err.name}: ${err.message}` + debug ? err.stack : "");
	} finally {
		// Do it again, no matter what happened. XXX There is currently no way to stop it
		fetchUpdates(logger, bot, offset, emitter, debug);
	}
}
...
function makeUpdateEmitter(logger, bot, settings) {
	...
	if (settings.telegram.skipOldMessages) {
		...
		if (settings.debug) {
			logger.log("Clearing old Telegram messages");
		}
		...
		p = clearInitialUpdates(bot)
			.then(() => {
				...
				if (settings.debug) {
					logger.log("Old Telegram messages cleared");
				}
			});
	}

	// Start the fetching
	p.then(() => fetchUpdates(logger, bot, 0, emitter, settings.debug));
