...
function setup(dcBot, tgBot, dcUsers, messageMap) {
	...
	dcBot.on("message", (message) => {
		...
		if (senderId !== dcBot.user.id) {
			...
			if (Application.chats.has("discord", message.channel.id)) {
				const chat = Application.chats.getChat("discord", message.channel.id);

				// Check for attachments and pass them on
				message.attachments.forEach(({url}) => {
					tgBot.sendMessage({
						chat_id: Number(chat.telegram),
						text: `<b>${senderName}</b>\n<a href="${url}">${url}</a>`,
						parse_mode: "HTML"
					  })
					  .catch((err) => Application.logger.error(`[${chat.name}] Telegram did not accept an attachment:`, err));
				});
				...
				message.embeds.forEach((embed) => {
					...
					tgBot.sendMessage({
					  ...
					  .catch((err) => Application.logger.error(`[${chat.name}] Telegram did not accept an embed:`, err));
				});
				...
				if (message.cleanContent) {
					...
					tgBot.sendMessage({
					  ...
					  .catch((err) => {
						Application.logger.error(`[${chat.name}] Telegram did not accept a message:`, err);
						Application.logger.error(`[${chat.name}] Failed message:`, err);
					  });
				}

			} else if (!Application.chats.has("discord-guild", message.channel.guild.id)) {	// Check if it is the correct server
	...
	dcBot.on("messageUpdate", (oldMessage, newMessage) => {
		...
		if (newMessage.author.id !== dcBot.user.id) {
			...
			if (Application.chats.has("discord", newMessage.channel.id)) {
				const chat = Application.chats.getChat("discord", newMessage.channel.id);
				...
				Promise.resolve()
				  ...
				  .catch((err) => Application.logger.error(`[${chat.name}] Could not edit Telegram message:`, err));
			}
	...
	dcBot.on("disconnected", (evt) => {
		...
		Application.chats.map.forEach(chat => {
			tgBot.sendMessage({
				chat_id: Number(chat.telegram),
				text: "**TEDICROSS**\nThe discord side of the bot disconnected! Please check the log"
			  })
			  .catch((err) => Application.logger.error(`[${chat.name}] Could not send message to Telegram:`, err));
		  });
	});
