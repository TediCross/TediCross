 ...
 * @private
 */
function makeJoinLeaveFunc(verb, bridgeMap, tgBot) {
		...
		member.guild.channels
			...
			.forEach(async (bridge) => {
				...
				try {
					...
					Application.logger.error(`[${bridge.name}] Could not notify Telegram about a user that ${verb} Discord`, err);
				}
			});
	};
}
 ...
 */
function setup(dcBot, tgBot, messageMap, bridgeMap, settings) {
	...
	dcBot.on("guildMemberAdd", makeJoinLeaveFunc("joined", bridgeMap, tgBot));

	// Listen for users joining the server
	dcBot.on("guildMemberRemove", makeJoinLeaveFunc("left", bridgeMap, tgBot));
