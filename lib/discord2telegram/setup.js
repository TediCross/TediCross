...
function setup(dcBot, tgBot, dcUsers, messageMap) {
	...
	dcBot.on("message", (message) => {
		...
		if (senderId !== dcBot.user.id) {
			...
			} else if (message.channel.guild === undefined || !Application.bridge.has("discord-guild", message.channel.guild.id)) {	// Check if it is the correct server
				// The message is from the wrong chat. Inform the sender that this is a private bot
				message.reply("This is an instance of a TediCross bot, bridging a chat in Telegram with one in Discord. If you wish to use TediCross yourself, please download and create an instance. You may ask @Suppen on Telegram for help. See also https://github.com/Suppen/TediCross");
			}
		}
	});
