		...
		getBridgesInGuild(member.guild, bridgeMap)
			...
			.forEach(async (bridge) => {
				...
				try {
					...
					Application.logger.error(`[${bridge.name}] Could not notify Telegram about a user that joined Discord`, err);
				}
			});
	});
		...
		getBridgesInGuild(member.guild, bridgeMap)
			...
			.forEach(async (bridge) => {
				...
				try {
					...
					Application.logger.error(`[${bridge.name}] Could not notify Telegram about a user that left Discord`, err);
				}
			});
	});
