...
function setup(logger, dcBot, tgBot, messageMap, bridgeMap, settings) {
	...
	dcBot.on("message", async (message) => {
		...
		if (bridges !== undefined) {
			...
			message.reply(
				"This is an instance of GoatBot, bridging a chat in Telegram with one in Discord. "
				+ "If you wish to use GoatBot yourself, please download and create an instance. "
				+ "See also https://github.com/Myke500/GoatBot"
			);
		}
	});
	...
	function onMessageDelete(message) {
		...
		bridges.forEach(async (bridge) => {
			try {
				...
				// Try to delete them
				await Promise.all(
					tgMessageIds.map((tgMessageId) => tgBot.deleteMessage({chat_id: bridge.telegram.chatId, message_id: tgMessageId}))
				);
			} catch (err) {
