 ...
 */
function setup(dcBot, tgBot, logger, settings, dcUsers) {
		...
		if (senderId !== dcBot.user.id) {
			...
			if (message.channel.id === settings.discord.channelID) {
				...
				.then(tgMessage => {
					...
					MessageMap.instance.insert(MessageMap.DISCORD_TO_TELEGRAM, message.id, tgMessage.message_id);
				})
				.catch(err => {
	...
	dcBot.on("messageUpdate", (oldMessage, newMessage) => {
		...
		if (newMessage.author.id !== dcBot.user.id) {
			...
			Promise.resolve()
			  .then(() => MessageMap.instance.getCorresponding(MessageMap.DISCORD_TO_TELEGRAM, newMessage.id))
			  .then(tgMessageId => {
				// Get info about the sender
