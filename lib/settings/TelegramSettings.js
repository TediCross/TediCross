...
class TelegramSettings {
	...
	constructor(settings) {
		...
		/**
		 * Whether we should also add the last name of the telegram user (only works if useFirstNameInsteadOfUsername is set to true)
		 *
		 * @type {Boolean}
		 */
		this.addLastName = settings.addLastName;
	...
	static validate(settings) {
		...
		// Check that addLastName is a boolean
		if (Boolean(settings.addLastName) !== settings.addLastName) {
			throw new Error("`settings.addLastName` must be a boolean");
		}
